export const HOMEPAGE = '/'
export const LOGIN = '/login'
export const REGISTER = '/register'
export const LOGOUT = '/logout'

export const USER_MANAGE_POSTS = '/user/manage-posts'
export const USER_PROFILE = '/user/manage-profile'
export const USER_POST_NEW = '/user/post-new'

export const POST_LIKED = '/post-liked'

export const ADMIN_MANAGE_POSTS = '/admin/manage-posts'
export const ADMIN_MANAGE_USERS = '/admin/manage-users'
export const ADMIN_MANAGE_CATEGORIES = '/admin/manage-categories'
export const ADMIN_MANAGE_ACREAGE = '/admin/manage-acreage'
export const ADMIN_MANAGE_PRICE = '/admin/manage-price'
